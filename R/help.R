...
#' }
#'
#' Parameterizes a model for a collection of noisy time-series indexed by domain or observation unit
#' as an additive function process plus noise process.  The latent functions are modeled under both
#' Gaussian process (GP) and intrinsic Gaussian Markov random (iGMRF) field priors.  Dependence is
#' estimated among the set of functions by selecting a prior, G, on the covariance or precision
#' parameters of the GP and iGMRF, respectively, where G receives a Dirichlet process (DP) prior.
#' The resulting marginal prior on the functions is a nonparametric scale mixture over the 
#' covariance or precision parameters, with G the unknown mixing measure.  Draws from a DP are
#' almost surely discrete, allowing for ties (interpreted as clusters) among the covariance or
#' precision parameters indexed by domain or observation unit.  Functions are included that
#' permit additional inference on the clustering properties over the domains. The mixture models allow
#' specification of multiple additive latent functions for each of the GP and iGMRF DP 
#' mixture formulations. Each function (under a GP prior) may be specified with a covariance 
#' function selected from a set of available options that allow for combinations of trend
#' and seasonal components.  The same is true for precision matrix 
...
#' The user may specify multiple latent functions in an additive regression formulation, where
#' each covariance kernel may be selected from the squared exponential (\code{"se"}), the
#' rational quadratic (\code{"rq"}), or a quasi-periodic (product of a period and squared exponential
...
#' parameter, \code{kappa}, that multiplies a fixed matrix, \code{Q}, which specifies the length-scale
#' of dependence.  The user may specify multiple functions in an additive formulation, each with its
#' own precision matrix.  The precision matrix is specified via two options.  Input \code{q_type} 
#' indicates whether the precision matrix is a \emph{trend}  (\code{"tr"}) precision matrix or a 
#' \emph{seasonal} (\code{"sn"}) precision matrix. Integer innput \code{q_order} controls the 
#' length scale of the estimated functions and specifies the 
...
#' and aggregates line plots of posterior mean estimates for member denoised functions.  An optional 
...
#' of \code{gpdpgrow()} or \code{gmrfdpgrow()} (where the model type, GP or iGMRF, is communicated
#' with input \code{model}) that compares credible intervals for covariance or precision
#' parameters where the data are drawn from an informative sampling design (rather than
#' as \emph{iid}), so that the distribution for the population is not the same as that for
...
#' to plot both estimated and predicted function values (with the option for credible intervals)
#' where the prediction interval is outside the range of data. The plot function, 
#' \code{\link{cluster_plot}}, should be used where the predicted values are in the range
...
#'
#' \code{\link{cps}} Data derived from the Current Population Survey (CPS) administered to households in
#' local areas within each of 51 states.  These data capture a rectangular matrix of monthly-indexed
#' all-other employment direct estimates computed from the set of household responses.  The data capture
#' 156 month observations (from 2000 - 2013) for each of the 51 states.  Two objects are included:
#' \code{y_raw} contains the 51 x 156 matrix of all-other employment counts.  \code{y} contains the
#' 51 x 156 matrix of all-other employment counts are standardization to (0,1), by state, to all
